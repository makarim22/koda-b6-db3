...
select * from movies m left join movies_directors ms on m.id = ms.movie_id inner join directors d on ms.director_id = d.id left join movies_genres mg on m.id = mg.movie_id limit 10;
...
---
select * from actors a left join roles r on a.id = r.actor_id left join movies m on r.movie_id = m.id limit 10;